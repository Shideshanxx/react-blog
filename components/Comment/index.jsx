/**
 * 评论组件，非常复杂
 * 1. 评论提交验证码，需要在后端存到redis中，暂时不弄
 * 2. 评论使用emoji插件，由于emoji提交到后端，数据不合法，无法存储，暂时不弄
 * 3. 该组件的关键点在于评论数据的处理，数据是一维的，如何变换成二维甚至三维的层叠评论数据
 */
...
const ReplyEditor = ({ props, hideRplyEditor, qureyComment }) => {
	...
	return (
		<div className="reply-editor-box" >
			<Input ref={inputRef} placeholder={`回复${props.userName}`} onChange={onValueChange} value={value} />
...
// 发布评论组件
const Editor = ({ onValueChange, setValue, onSubmit, submitting, value }) => {
