...
import './style.less';
const { Search } = Input;
const darkPath = '//cdn.zjutshideshan.cn/blog_static/darkTheme.css';
...
const Header = (props) => {
  ...
  useEffect(() => {
    // 改用本地css进行快速暗黑模式适配 无需再请求接口动态分析， 问题：如果后面新增的页面无法适配，请用 DarkReader.exportGeneratedCSS(); 重新导出css重写darkTheme.css
    ...
    // 判断localStorage是否有用户信息 有则存入redux
    ...
    jsonp({
      ...
      data: {
        ak: 'hK7QwExpD79tuldrRZozaX5PNicKyIH5', // 百度地图ak
        coor: 'gcj02'
      },
      success: (res) =>{
  ...
  const writeArticle = () => {
    if (~~props.userInfo.auth > 0) {
      window.open('http://admin.zjutshideshan.cn')
    } else {
    ...
    <div id="weather-content">
      <If condition={weatherInfo && weatherInfo.now}>
        <div className="info1">
          ...
          <div className="right">
            <img src={`//cdn.zjutshideshan.cn/blog_image/${weatherInfo.now.icon}.png`} />
          </div>
        </div>
            ...
            <ul className="list">
              <For each="item" index="index" of={weatherInfo.hourly.slice(0, 7)}>
                <li className="item" key={index}>
                  <img src={`//cdn.zjutshideshan.cn/blog_images/weather2/${item.icon}.png`} />
          ...
          <div className="future">
            ...
            <ul className="list">
              <For each="item" index="index" of={weatherInfo.daily}>
                <li className="item" key={index}>
                  <img src={`//cdn.zjutshideshan.cn/blog_images/weather2/${item.iconDay}.png`} />
        ...
        <p style={{ fontSize: 12, textAlign: 'center', color: '#999', margin: '20px 0 10px' }}>
          和风天气 - 实况天气推送： {moment(weatherInfo.updateTime).startOf('min').fromNow()}
        </p>
      </If>
          ...
          <If condition={ipWeather}>
            <Popover placement="bottom" content={weatherContent} trigger="hover">
              <div className='weather-title-box'>
                ...
                <If condition={weatherInfo && weatherInfo.now}>
                  <img style={{ width: 20, height: 20, marginRight: 5 }} src={`//cdn.zjutshideshan.cn/blog_image/${weatherInfo.now.icon}.png`} />
